...
def get_envelope_args(signer_email, signer_name):
...
def create_all_envelopes(jwt_values):
    ...
    with open(os.getcwd() + '\\Mckenneys_Esign\\CSV_Folder\\SPdump.csv', mode='r', encoding='utf-8-sig') as file:
        ...
        for row in reader:
            ...
            if index < get_row_length():
                # Build args objects
                envelope_args = get_envelope_args(email, first_name)
                args = get_args(jwt_values["api_account_id"], jwt_values["access_token"], jwt_values["base_path"], envelope_args)
...
def check_statuses(args):
    ...
    envelope_statuses = Eg002SigningViaEmailController.status_changes(args, id_list)
    ...
    for envelope in envelope_statuses.envelopes:
        ...
        if envelope.status == 'completed':
            ...
            Eg002SigningViaEmailController.download_documents(args, envelope_id)
        ...
        if not signed_enevlope_list:
            ...
            signed_envelopes.to_csv(os.getcwd() + '\\Mckenneys_Esign\\CSV_Folder\\signed_envelopes.csv', index=False)
...
def create_mvr_rows():
    ...
    combined_column['Employee Name'] = combined_column['FirstName'].astype(str) + combined_column['LastName'].astype(str)
    ...
    #Renaming columns
    arrange_column = arrange_column.rename(columns={'EmpID': 'Employee ID', 'SupervisorName' : 'Requestor', 'VehicleAssignmentStatus' : 'Status'})

    #Concats, shifts, and udpates columns
    updated = pd.concat([combined_column['Employee Name'], arrange_column], axis=1)
    updated = updated.reindex(columns=['Employee Name','Employee ID','Requestor','Status','Envelope ID'])
    updated['Requestor'] = updated['Requestor'].astype(str)
    updated['Requestor'] = updated['Requestor'].str.lower()
    updated['Requestor'] = updated['Requestor'].apply(lambda x: '.' .join(x.split()) + "@mckenneys.com")
    updated.loc[:, 'Status'] = "Awaiting Signature"
    updated.to_csv(os.getcwd() + '\\Mckenneys_Esign\\CSV_Folder\\envelopes_sent.csv', index=False)
...
def get_envelope_ids():
    ...
    num_of_awaiting_signaturesre = check_status[check_status['Status'] == 'Awaiting Signature']

    envelope_ids = num_of_awaiting_signaturesre['Envelope ID'].tolist()

    envelope_ids = [null_value for null_value in envelope_ids if str(null_value) != 'nan']
...
def main():
    ...
    # Build auth data
    private_key = private_key.encode("ascii").decode("utf-8") # idk why its encoding and decoding, but docusign said to do so
    ...
    if os.path.exists(sp_dump_path):
        ...
        create_all_envelopes(jwt_values)
